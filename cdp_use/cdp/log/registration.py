...
class LogRegistration:
    ...
    def entryAdded(
        self,
        callback: Union[Callable[['EntryAddedEvent', Optional[str]], None], Callable[['EntryAddedEvent', Optional[str]], Awaitable[None]]],
        once: bool = False,
    ) -> None:
        """
        Register or unregister a callback for entryAdded events.
        ...
        if self._mode == 'register':
            self._registry.register("Log.entryAdded", callback, once)
        else:  # unregister mode
            self._registry.unregister("Log.entryAdded", callback)
